...
class DetectorManager:
    def __init__(self):
        ...
        self.desired_speed = 0.35  # m/s, reference speed
        ...
        # Edge detection result publish
        self.edge_pub = rospy.Publisher('/mako_1/image_rect', Image, queue_size=1)
    ...
    def imageCb(self, data):
        try:
            if self.i == 0:
                ...
                self.image_vis = cv2.resize(cv_image, (1280, 720), interpolation=cv2.INTER_LINEAR)
                # self.image_vis[0:500, :] = [0,0,0]
                ...
                my_detector._detect_edge(self.image_vis)
                my_detector._get_masked_image()
                binary_seg_image = my_detector.masked_edges_image
                # self.edge_pub.publish(binary_seg_image)
                # cv2.imwrite('gray.png',binary_seg_image)
                postprocess_result = self.postprocessor.postprocess(
                ...
                # print(fit_params)
                if fit_params != []:
                    index = np.where((fit_params[:,2] >=600))
    ...
    def start_pp(self):
            ...
            if self.filtered_fit_params.shape[0] == 2:
                ...
                v = 0
                for i in range(2):
                    y_near = int(self.filtered_fit_params[i,2])
                    y_far = int(self.filtered_fit_params[i,3])
                    y_far = y_far+(y_near-y_far)*0.35                                                                                                                                                                                                                                                                                             
                    fit_x_near = int(self.filtered_fit_params[i,0] * y_near + self.filtered_fit_params[i,1])
                    fix_x_far = int(self.filtered_fit_params[i,0] * y_far + self.filtered_fit_params[i,1])
                    u += fix_x_far
                    v += y_far
                u = int(u//2)
                v = int(v//2)
                ...
                theta = np.arctan(-1/k)
                if self.filtered_fit_params[0,0] <= 0: #left
