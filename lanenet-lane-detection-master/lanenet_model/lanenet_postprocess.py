...
class _LaneNetCluster(object):
    ...
    def _embedding_feats_dbscan_cluster(self, lane_coordinate):
        """
        dbscan cluster
        ...
        ret = {
            # 'origin_lane_coordinate': lane_coordinate,
    ...
    def _get_lane_embedding_feats(binary_seg_ret, instance_seg_ret):
        ...
        lane_coordinate = np.array(np.vstack([idx[1], idx[0]]).transpose())

        # assert lane_embedding_feats.shape[0] == lane_coordinate.shape[0]

        ret = {
            'lane_embedding_feats': instance_seg_ret,
    ...
    def apply_lane_feats_cluster(self, binary_seg_result, instance_seg_result):
        ...
        dbscan_cluster_result = self._embedding_feats_dbscan_cluster(
            lane_coordinate=get_lane_embedding_feats_result['lane_coordinates']
        )
...
class LaneNetPostProcessor(object):
    ...
    def postprocess(self, binary_seg_result, instance_seg_result=None,
        ...
        binary_seg_result[0:500, :] = 0
        binary_seg_result = np.array(binary_seg_result, dtype=np.uint8)
        # cv2.imwrite('gray_01.png',binary_seg_result)
        # # apply image morphology operation to fill in the hold and reduce the small area
        # morphological_ret = _morphological_process(binary_seg_result, kernel_size=5)
        # # print(morphological_ret.shape)
        # cv2.imwrite('gray_02.png',morphological_ret)
        ...
        # cv2.imwrite('gray_03.png',morphological_ret)
        # apply embedding features cluster
        mask_image, lane_coords = self._cluster.apply_lane_feats_cluster(
            binary_seg_result=binary_seg_result,
        ...
        for lane_index, coords in enumerate(lane_coords):
            if data_source == 'tusimple':
                ...
                tmp_mask[tuple((np.int_(coords[:, 1] * 720 / 720), np.int_(coords[:, 0] * 1280 / 1280)))] = 255
            else:
            ...
            plot_y = np.linspace(int(min(nonzero_y)), int(max(nonzero_y)), int(max(nonzero_y)) - int(min(nonzero_y)))
            ...
            y_near = int(max(nonzero_y))
            y_far = int(min(nonzero_y))                                                                                                                                                                                                                                                                                             
            fit_x_near = int(fit_param[0] * y_near + fit_param[1])
            fix_x_far = int(fit_param[0] * y_far + fit_param[1])
            fit_param = [fit_param[0], fit_param[1], y_near, y_far, fit_x_near, fix_x_far]
            fit_params.append(fit_param)

            lane_pts = []
            for index in range(0, plot_y.shape[0], 1): #
                ...
                src_x = int(np.clip(fit_x[index], 0, ipm_image_width - 1))
                src_y = int(plot_y[index])
                lane_pts.append([src_x, src_y])
            lane_color = self._color_map[lane_index].tolist()
            for xy_coordinates in lane_pts:
                cv2.circle(source_image, (int(xy_coordinates[0]),
                                            int(xy_coordinates[1])), 5, lane_color, -1)
